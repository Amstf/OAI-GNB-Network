...
static void nr_rrc_process_reconfiguration_v1530(NR_UE_RRC_INST_t *rrc, NR_RRCReconfiguration_v1530_IEs_t *rec_1530, int gNB_index)
{
  ...
  if (rec_1530->masterKeyUpdate) {
    ...
    /** NOTE: this is a workaround to fix PDCP counters misalignment
     * on the UE after security keys update, e.g. during handover. */
    for (int j = 1; j <= MAX_DRBS_PER_UE; j++) {
      if (get_DRB_status(rrc, j) != RB_NOT_PRESENT) {
        nr_pdcp_reset_drb_counters(rrc->ue_id, j);
      }
    }
  }
