...
extern const neighbour_cell_configuration_t *get_neighbour_cell_config(const gNB_RRC_INST *rrc, uint64_t cell_id);

static const nr_neighbour_cell_t *find_neighbour_by_pci_any_cell(const gNB_RRC_INST *rrc,
                                                                 uint32_t neighbour_pci,
                                                                 const neighbour_cell_configuration_t **owner)
{
  if (!rrc->neighbour_cell_configuration)
    return NULL;

  seq_arr_t *cells = rrc->neighbour_cell_configuration;
  size_t num_cells = seq_arr_size(cells);
  for (size_t i = 0; i < num_cells; ++i) {
    const neighbour_cell_configuration_t *cell =
        (const neighbour_cell_configuration_t *)seq_arr_at(cells, i);
    if (cell == NULL || cell->neighbour_cells == NULL)
      continue;

    seq_arr_t *neighbours = cell->neighbour_cells;
    size_t num_neigh = seq_arr_size(neighbours);
    for (size_t n = 0; n < num_neigh; ++n) {
      const nr_neighbour_cell_t *neighbour =
          (const nr_neighbour_cell_t *)seq_arr_at(neighbours, n);
      if (neighbour->physicalCellId == (int)neighbour_pci) {
        if (owner != NULL)
          *owner = cell;
        return neighbour;
      }
...
void nr_HO_N2_trigger_telnet(gNB_RRC_INST *rrc, uint32_t neighbour_pci, uint32_t rrc_ue_id)
{
  ...
  const nr_neighbour_cell_t *neighbour = NULL;

  if (cell == NULL) {
    LOG_W(NR_RRC,
          "N2 HO trigger: serving cell %ld has no dedicated neighbour configuration; attempting global lookup\n",
          (long)scell_du->nr_cellid);
    neighbour = find_neighbour_by_pci_any_cell(rrc, neighbour_pci, &cell);
    if (neighbour == NULL) {
      LOG_E(NR_RRC,
            "N2 HO trigger failed for UE %d: could not locate neighbour PCI=%d in any configuration\n",
            rrc_ue_id,
            neighbour_pci);
      return;
    }
    if (cell && cell->nr_cell_id != scell_du->nr_cellid) {
      LOG_W(NR_RRC,
            "N2 HO trigger: neighbour PCI=%d is configured under cell ID %ld instead of serving cell %ld\n",
            neighbour_pci,
            (long)cell->nr_cell_id,
            (long)scell_du->nr_cellid);
    }
  } else {
    neighbour = get_neighbour_cell_by_pci(cell, neighbour_pci);
    if (neighbour == NULL) {
