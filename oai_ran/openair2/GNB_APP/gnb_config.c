...
void RCconfig_nr_macrlc(configmodule_interface_t *cfg)
{
  ...
  if (MacRLC_ParamList.numelt > 0) {
    ...
    for (j = 0; j < RC.nb_nr_macrlc_inst; j++) {
      ...
      const char *slice_conf_path = NULL;
      if (config_isparamset(MacRLC_ParamList.paramarray[j], MACRLC_SLICE_CONF_IDX))
        slice_conf_path = *MacRLC_ParamList.paramarray[j][MACRLC_SLICE_CONF_IDX].strptr;

      nssai_t slice_list[MAX_NUM_SLICE] = {0};
      uint16_t configured_slice_count = set_snssai_config(slice_list, MAX_NUM_SLICE, 0, 0);
      nr_mac_configure_slices(RC.nrmac[j], slice_list, configured_slice_count, slice_conf_path);
      // PRB Blacklist
      uint16_t prbbl[MAX_BWP_SIZE] = {0};
      int num_ulprbbl = get_prb_blacklist(j, prbbl);
      if (num_ulprbbl != -1) {
