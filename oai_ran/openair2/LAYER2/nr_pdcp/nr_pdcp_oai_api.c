...
void nr_pdcp_reset_drb_counters(ue_id_t ue_id, rb_id_t rb_id)
{
  nr_pdcp_manager_lock(nr_pdcp_ue_manager);
  ...
  if (entity == NULL) {
    nr_pdcp_manager_unlock(nr_pdcp_ue_manager);
    LOG_E(PDCP, "no entity found (ue_id %ld, DRB %ld)\n", ue_id, rb_id);
    return;
  }

  LOG_I(PDCP, "UE %ld: Reset PDCP counters for entity %ld\n", ue_id, rb_id);
  entity->tx_next = 0;
  entity->rx_next = 0;
  entity->rx_deliv = 0;
  nr_pdcp_manager_unlock(nr_pdcp_ue_manager);
