...
/*
 * Maximum number of slices handled inside the NR MAC scheduler. The default
 * slice (SID 0) consumes one slot in the slice table, therefore the number of
 * configurable S-NSSAIs is bounded by MAX_NUM_SLICE - 1.
 */
#define MAX_NUM_SLICE 8

/*
 * Maximum number of PDU sessions per UE that the slice-aware scheduler tracks
 * concurrently.  This value mirrors the limit enforced in the Slice branch and
 * allows the per-UE slice bookkeeping arrays to have a deterministic size.
 */
