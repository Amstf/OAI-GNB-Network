...
static uint32_t bytes_for_slice(const NR_UE_sched_ctrl_t *sched_ctrl, int16_t sid);
...
static void nr_store_dl_slice_info(module_id_t module_id)
{
  ...
  UE_iterator(UE_info->connected_ue_list, UE) {
    ...
    for (int i = 0; i < seq_arr_size(&sched_ctrl->lc_config); ++i) {
      ...
      for (uint8_t sl = 0; sl < slice_info->count; ++sl) {
        NR_slice_info_t *entry = slice_info->list[sl];
        if (!entry)
          continue;
        if (entry->nssai.sst == lc->nssai.sst && entry->nssai.sd == lc->nssai.sd) {
          sched_ctrl->dl_sl_info[lcid].sid = entry->sid;
          break;
        }
      }
    }
...
static void nr_get_ue_active_slice_list(module_id_t module_id, frame_t frame, slot_t slot)
{
  (void)frame;
  (void)slot;
  ...
  UE_iterator(UE_info->connected_ue_list, UE) {
    ...
    for (int idx = 0; idx < MAX_NUM_PDU_SESSION + 1; ++idx) {
      sched_ctrl->avail_slice_list[idx].sid = -1;
      sched_ctrl->avail_slice_list[idx].bytes = 0;
    }

    sched_ctrl->avail_slice_list[0].sid = 0;
    sched_ctrl->num_slice_d = 1;

    uint16_t slice_counts[MAX_NUM_PDU_SESSION + 1] = {0};

    for (int i = 0; i < seq_arr_size(&sched_ctrl->lc_config); ++i) {
      const nr_lc_config_t *lc = seq_arr_at(&sched_ctrl->lc_config, i);
      const int sid = sched_ctrl->dl_sl_info[lc->lcid].sid;
      if (sid > 0 && sid < MAX_NUM_PDU_SESSION + 1)
        slice_counts[sid]++;
    }

    int cursor = 1;
    for (int sid = 1; sid < MAX_NUM_PDU_SESSION + 1; ++sid) {
      if (slice_counts[sid] == 0)
        continue;
      sched_ctrl->avail_slice_list[cursor].sid = sid;
      ++cursor;
    }

    sched_ctrl->num_slice_d = cursor;
  }
}

static void nr_slice_preprocess(module_id_t module_id, frame_t frame, slot_t slot)
{
  (void)frame;
  gNB_MAC_INST *mac = RC.nrmac[module_id];
  NR_UEs_t *UE_info = &mac->UE_info;
  ...
  UE_iterator(UE_info->connected_ue_list, UE) {
    NR_UE_sched_ctrl_t *ctrl = &UE->UE_sched_ctrl;
    ...
    const int16_t previous = ctrl->slice_for_this_sched;
    const int16_t selected = nr_mac_select_slice_for_ue(ctrl, slot, ue_index);
    AssertFatal(selected >= 0, "Slice selection failed for UE %04x", UE->rnti);
    ctrl->slice_for_this_sched = selected;

    if (previous != selected) {
      const NR_slice_info_t *slice = NULL;
      for (uint8_t sl = 0; sl < slice_info->count; ++sl) {
        const NR_slice_info_t *candidate = slice_info->list[sl];
        if (candidate && candidate->sid == selected) {
          slice = candidate;
          break;
        }
      }
      ...
      if (slice)
        LOG_I(NR_MAC,
              "UE %04x active slice -> %d (SST %u, SD 0x%06x)",
              UE->rnti,
              selected,
              slice->nssai.sst,
              slice->nssai.sd);
      else
        LOG_I(NR_MAC, "UE %04x active slice -> %d", UE->rnti, selected);
    }
...
static void slice_prb_estimate(module_id_t module_id,
                               frame_t frame,
                               slot_t slot,
                               NR_UE_info_t **UE_list,
                               SLsched_t *slice_sched)
{
  (void)frame;
  gNB_MAC_INST *mac = RC.nrmac[module_id];
  NR_Slices_t *slice_info = &mac->SL_info;
  ...
  int idx = 0;
  for (uint8_t sl = 0; sl < slice_info->count; ++sl) {
    NR_slice_info_t *slice = slice_info->list[sl];
    if (!slice)
      continue;

    SLsched_t *entry = &slice_sched[idx++];
    entry->slice = slice;
    entry->prb_estimate = 0;
    entry->dedicated_prbs = 0;
    entry->minimum_prbs = 0;
    entry->max_prbs = 0;
    entry->used_prbs = 0;
    ...
    UE_iterator(UE_list, UE) {
      ...
      if (sched_ctrl->slice_for_this_sched != slice->sid)
        continue;

      if (sched_ctrl->ul_failure)
        continue;

      const uint32_t bytes = bytes_for_slice(sched_ctrl, slice->sid);
      if (bytes == 0)
        continue;

      NR_UE_DL_BWP_t *dl_bwp = &UE->current_DL_BWP;
      const int coresetid = sched_ctrl->coreset->controlResourceSetId;
      const uint16_t bwpSize = coresetid == 0 && mac->cset0_bwp_size > 0 ? mac->cset0_bwp_size : dl_bwp->BWPSize;

      uint8_t mcs = sched_ctrl->dl_bler_stats.mcs;
      if (sched_ctrl->dl_bler_stats.last_frame == 0 && mcs == 0)
        mcs = 9;

      const uint8_t layers = get_dl_nrOfLayers(sched_ctrl, dl_bwp->dci_format);
      ...
      NR_pdsch_dmrs_t dmrs_parms = get_dl_dmrs_params(scc, dl_bwp, &tda_info, layers);

      uint32_t tbs = 0;
      uint16_t rbSize = 0;
      const int overhead = 12; // conservative MAC header overhead (4 PDUs)
      nr_find_nb_rb(Qm,
                    R,
                    1,
                    layers,
...
static void pf_dl_slice(module_id_t module_id,
                        ...
                        int num_beams,
                        int n_rb_sched[],
                        int remain_ues_per_beam[],
                        SLsched_t *slice_sched)
{
  gNB_MAC_INST *mac = RC.nrmac[module_id];
  NR_ServingCellConfigCommon_t *scc = mac->common_channels[0].ServingCellConfigCommon;
  NR_slice_info_t *slice = slice_sched->slice;
  if (!slice)
    return;
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (sched_ctrl->ul_failure || sched_ctrl->alreadySched)
      continue;

    if (sched_ctrl->slice_for_this_sched != slice->sid)
      continue;

    uint32_t slice_bytes = bytes_for_slice(sched_ctrl, slice->sid);
#ifdef E2_AGENT
    if (e2_agent_db != NULL && e2_agent_db->true_gbr && UE->is_GBR &&
        UE->guaranteed_tbs_bytes_dl > slice_bytes)
      slice_bytes = UE->guaranteed_tbs_bytes_dl;
#endif
    if (slice_bytes == 0 && frame != (sched_ctrl->ta_frame + 100) % 1024)
      continue;
    ...
    sched_pdsch->pm_index = get_pm_index(mac,
                                         UE,
    ...
    float denom = UE->dl_thr_ue > 1.0f ? UE->dl_thr_ue : 1.0f;
    float coeff = (float)tbs / denom;
#ifdef E2_AGENT
    if (e2_agent_db != NULL && e2_agent_db->true_gbr && UE->is_GBR) {
      const float guaranteed = UE->guaranteed_tbs_bytes_dl > 0 ? (float)UE->guaranteed_tbs_bytes_dl : 1.0f;
      const float gbr_coeff = guaranteed / denom;
      if (gbr_coeff > coeff)
        coeff = gbr_coeff;
    }
#endif

    UE_sched[curUE].coef = coeff;
    UE_sched[curUE].slice_bytes = slice_bytes;
  ...
  while (iterator->UE != NULL) {
    NR_UE_info_t *ue = iterator->UE;
    NR_UE_sched_ctrl_t *sched_ctrl = &ue->UE_sched_ctrl;
    NR_sched_pdsch_t *sched_pdsch = &sched_ctrl->sched_pdsch;
    NR_UE_DL_BWP_t *dl_bwp = &ue->current_DL_BWP;
    NR_UE_UL_BWP_t *ul_bwp = &ue->current_UL_BWP;
    uint32_t slice_bytes = iterator->slice_bytes;

    const int slice_remaining = slice_sched->max_prbs - slice_sched->used_prbs;
    if (slice_remaining < min_rbSize)
      break;
    ...
    if (remain_ues_per_beam[beam.idx] == 0 || n_rb_sched[beam.idx] < min_rbSize) {
    ...
    uint16_t *rballoc_mask = mac->common_channels[0].vrb_map[beam.idx];
    ...
    const uint16_t slbitmap = SL_to_bitmap(sched_pdsch->tda_info.startSymbolIndex,
    ...
    uint32_t TBS = 0;
    uint16_t rbSize = 0;
    const int overhead = 3 * 4 + 2 * (frame == (sched_ctrl->ta_frame + 100) % 1024);
    ...
    n_rb_sched[beam.idx] -= rbSize;
    slice_sched->used_prbs += rbSize;

    for (int rb = 0; rb < rbSize; ++rb)
      rballoc_mask[rb + sched_pdsch->rbStart + bwp_start] |= slbitmap;

    remain_ues_per_beam[beam.idx]--;
    sched_ctrl->alreadySched = true;
    ++iterator;
  }

  slice_sched->minimum_prbs = slice_sched->used_prbs;
}
                          ...
                          SLsched_t *slice_sched)
{
  ...
  int remainUEs[MAX_NUM_BEAM_PERIODS];
  for (int b = 0; b < num_beams; ++b)
    remainUEs[b] = max_num_ue;

  UE_iterator(UE_list, UE) {
    NR_UE_sched_ctrl_t *sched_ctrl = &UE->UE_sched_ctrl;
    sched_ctrl->alreadySched = false;
    ...
    if (remainUEs[beam.idx] == 0)
      continue;

    bool ret = allocate_dl_retransmission(module_id,
                                          frame,
                                          slot,
                                          &n_rb_sched[beam.idx],
                                          UE,
  ...
  int total_prbs = 0;
  int original_prbs[MAX_NUM_BEAM_PERIODS] = {0};
  for (int b = 0; b < num_beams; ++b) {
    original_prbs[b] = n_rb_sched[b];
    total_prbs += n_rb_sched[b];
  }
  ...
  if (capped_total < total_prbs) {
    const long long denom = total_prbs > 0 ? total_prbs : 1;
    int new_sum = 0;
    for (int b = 0; b < num_beams; ++b) {
      long long scaled = (long long)original_prbs[b] * capped_total / denom;
      if (scaled > original_prbs[b])
        scaled = original_prbs[b];
      n_rb_sched[b] = (int)scaled;
      new_sum += n_rb_sched[b];
    }

    int deficit = capped_total - new_sum;
    for (int b = 0; b < num_beams && deficit > 0; ++b) {
      const int available = original_prbs[b] - n_rb_sched[b];
      if (available <= 0)
        continue;
      const int grant = available < deficit ? available : deficit;
      n_rb_sched[b] += grant;
      deficit -= grant;
    }
  ...
  nr_mac_prepare_slice_budgets(total_prbs, slice_sched, RC.nrmac[module_id]->dl_num_slice);

  for (int i = 0; i < RC.nrmac[module_id]->dl_num_slice; ++i) {
    if (!slice_sched[i].slice)
      continue;
    ...
    total_prbs -= slice_sched[i].used_prbs;
  }
}
...
static int compare_slice_demand(const void *lhs, const void *rhs)
{
  const SLsched_t *a = lhs;
  const SLsched_t *b = rhs;
  return (a->prb_estimate > b->prb_estimate) - (a->prb_estimate < b->prb_estimate);
}

static uint32_t bytes_for_slice(const NR_UE_sched_ctrl_t *sched_ctrl, int16_t sid)
{
  for (int idx = 0; idx < sched_ctrl->num_slice_d; ++idx) {
    if (sched_ctrl->avail_slice_list[idx].sid == sid)
      return sched_ctrl->avail_slice_list[idx].bytes;
  }
  return 0;
}
...
static void nr_dlsch_preprocessor(module_id_t module_id, frame_t frame, slot_t slot)
{
  ...
  int n_rb_sched[MAX_NUM_BEAM_PERIODS];
  for (int i = 0; i < num_beams; ++i)
    n_rb_sched[i] = bw;

  if (mac->dl_num_slice <= 1) {
  ...
  SLsched_t slice_sched[MAX_NUM_SLICE + 1] = {0};
  slice_prb_estimate(module_id, frame, slot, UE_info->connected_ue_list, slice_sched);
  qsort(slice_sched, mac->dl_num_slice, sizeof(SLsched_t), compare_slice_demand);
