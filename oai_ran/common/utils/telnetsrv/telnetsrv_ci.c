...
int rrc_gNB_trigger_n2_ho(char *buf, int debug, telnet_printfunc_t prnt)
{
  ...
  if (!buf) {
    ...
    char *endptr = NULL;
    errno = 0;
    uint32_t ueId = strtoul(token, &endptr, 10);
    rrc_gNB_ue_context_t *ue_p = NULL;

    if (errno == 0 && *endptr == '\0') {
      ue_p = rrc_gNB_get_ue_context(RC.nrrrc[0], ueId);
      if (!ue_p)
        ue_p = rrc_gNB_get_ue_context_by_rnti_any_du(RC.nrrrc[0], ueId);
    }
    ...
    if (!ue_p) {
      ...
      if (looks_hex) {
        errno = 0;
        uint32_t rnti = strtoul(token, &endptr, 16);
        if (errno == 0 && *endptr == '\0') {
          ue_p = rrc_gNB_get_ue_context_by_rnti_any_du(RC.nrrrc[0], rnti);
          if (ue_p)
            ueId = ue_p->ue_context.rrc_ue_id;
        }
      }
    }

    if (!ue_p)
      ERROR_MSG_RET("UE with id/RNTI %s not found\n", token);
