...
void handleTimer(E2Sim* e2sim, int* timer, long* ric_req_id, long* ric_instance_id,
  ...
  E2SIM_DBG_FPRINTF(stderr, "Handle timer %d seconds, ricReqId %ld\n", timer[0], ric_req_id[0]);
  ...
  if (OAI_PROTOBUF) {
    ...
    E2SIM_DBG_FPRINTF(stderr, "periodicDataReportOaiProtobuf thread created successfully\n");
  }
  else {
    std::thread(periodicDataReport, e2sim, timer, seq_num, ric_req_id, ric_instance_id, 
      ran_function_id, action_id).detach();
    E2SIM_DBG_FPRINTF(stderr, "periodicDataReport thread created successfully\n");
  }
}
...
void periodicDataReport(E2Sim* e2sim, int* timer, long seqNum, long* ric_req_id, long* ric_instance_id,
  ...
  E2SIM_DBG_FPRINTF(stderr, "timer expired for requestorId %ld, instanceId %ld, ranFunctionId %ld, actionId %ld: %d s\n",
  ...
  if (DEBUG) {
    E2SIM_DBG_FPRINTF(stderr, "DEBUG mode\n");
  ...
  // send PDU
  if (payload) {
    E2SIM_DBG_FPRINTF(stderr, "Sending\n%s\n", payload);
    E2SIM_DBG_FPRINTF(stderr, "Encoding RIC Indication Report\n");
    // encoding::generate_e2apv1_indication_report(e2ap_pdu, payload, strlen(payload), ric_req_id[0], 0, 0, 0);
    // E2SIM_DBG_FPRINTF(stderr, "RIC Indication Report successfully encoded\n");
                        ...
                        long *ran_function_id, long *action_id)
{
  ...
  while (report_data_nrt_ric)
  {
    ...
    E2SIM_DBG_FPRINTF(stderr, "timer expired for requestorId %ld, instanceId %ld, ranFunctionId %ld, actionId %ld: %d s\n",
            requestorId, instanceId, ranFunctionId, actionId, timer[0]);
    E2SIM_DBG_FPRINTF(stderr, "Sending indication request buffer to gnb, size %d\n", indication_request_length);
    
    E2SIM_DBG_FPRINTF(stderr,"about to print buffer in periodicDataReportOaiProtobuf\n");
    for(int i = 0; i < indication_request_length; i++){
      E2SIM_DBG_FPRINTF(stderr,"---%hhx\n", indication_request_buffer[i]);
    }
    ...
    E2SIM_DBG_FPRINTF(stderr, "Waiting for response from gnb...");
    std::this_thread::sleep_for(sleep_duration);
    recvlen = in_socket.receive_from(boost::asio::buffer(recvbuf), remote_endpoint_in);
    E2SIM_DBG_FPRINTF(stderr, " recevied %lu bytes\n", recvlen);

    // append null terminator to payload such that the buffer can be processed by the kind-of-stupid following encoders
    recvbuf[recvlen++] = '\0';

    E2SIM_DBG_FPRINTF(stderr,"printing buf recevied from gnb:\n");
    for(int i = 0; i < recvlen; i++){
      E2SIM_DBG_FPRINTF(stderr, " %hhx ", recvbuf[i]);
    }
    E2SIM_DBG_FPRINTF(stderr, "\n");

    E2SIM_DBG_FPRINTF(stderr,"Sending report to ric\n");
...
void log_message(char* message, char* message_type, int len) {
  ...
  if (fp == NULL) {
    E2SIM_DBG_PRINTF("ERROR: fp is NULL\n");
    return;
  }
  ...
  E2SIM_DBG_PRINTF("%s,%03d\t%s\t%d\t%s\n", buffer, millisec, message_type, len, msg_copy);
  E2SIM_DBG_FPRINTF(fp, "%s,%03d\t%s\t%d\t%s\n", buffer, millisec, message_type, len, msg_copy);
...
void stop_data_reporting_nrt_ric(void) {
  E2SIM_DBG_PRINTF("Terminating data reporting to near-real-time RIC\n");
...
void send_msg_buffer_to_bs(uint8_t *buffer, int buflen) {
  E2SIM_DBG_FPRINTF(stderr, "sending udp datagram\n");
  boost::asio::io_service io_service;
