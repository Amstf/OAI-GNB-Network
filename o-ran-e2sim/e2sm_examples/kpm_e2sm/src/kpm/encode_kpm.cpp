...
void encode_kpm_function_description(E2SM_KPM_RANfunction_Description_t* ranfunc_desc) {
  ...
  E2SIM_DBG_XER(stderr, &asn_DEF_E2SM_KPM_RANfunction_Description, ranfunc_desc);
...
void encode_e2sm_kpm_indication_header(E2SM_KPM_IndicationHeader_t *ihead, uint8_t *plmnid_buf, uint8_t *sst_buf, uint8_t *sd_buf,
  ...
  */  
  ...
  E2SIM_DBG_XER(stderr, &asn_DEF_E2SM_KPM_IndicationHeader, ihead);
...
void encode_kpm_ocuup_user_level(RAN_Container_t *ranco,
  ...
  E2SIM_DBG_XER(stderr, &asn_DEF_RAN_Container, ranco);
...
void encode_kpm_ocucp_user_level(RAN_Container_t *ranco,
  ...
  E2SIM_DBG_FPRINTF(stderr,"printing serving_buf (len %d) recevied from gnb:\n", serving_buf_len);
  for(int i = 0; i < serving_buf_len; i++){
    E2SIM_DBG_FPRINTF(stderr, " %hhx ", serving_buf[i]);
  }
  E2SIM_DBG_FPRINTF(stderr, "\n");

  E2SIM_DBG_FPRINTF(stderr,"printing servingstr->buf (len %d) recevied from gnb:\n", servingstr->size);
  for(int i = 0; i < servingstr->size; i++){
    E2SIM_DBG_FPRINTF(stderr, " %hhx ", servingstr->buf[i]);
  }
  E2SIM_DBG_FPRINTF(stderr, "\n");
...
void encode_kpm_odu_user_level(RAN_Container_t *ranco,
  ...
  *p_prb_usage_ul = (long)prb_usage_ul;
  ...
  E2SIM_DBG_PRINTF("enc5\n");
  
  ue_report_item->ul_PRBUsage = p_prb_usage_ul;

  E2SIM_DBG_PRINTF("enc6\n");
...
void encode_kpm_report_rancontainer_du_parameterized(E2SM_KPM_IndicationMessage_t* indicationmessage,
  ...
  E2SIM_DBG_PRINTF("After creating RAN container, xer printing it\n");
  E2SIM_DBG_XER(stderr, &asn_DEF_RAN_Container, ranco);
  ...
  E2SIM_DBG_XER(stderr, &asn_DEF_E2SM_KPM_IndicationMessage, indicationmessage);
  ...
  E2SIM_DBG_FPRINTF(stderr, "exiting of encode_kpm_report_rancontainer_du_parameterized\n");
}
...
void encode_kpm_report_rancontainer_cuup_parameterized(E2SM_KPM_IndicationMessage_t* indicationmessage,
  ...
  E2SIM_DBG_FPRINTF(stderr, "After creating RAN container, xer printing it\n");
  E2SIM_DBG_XER(stderr, &asn_DEF_RAN_Container, ranco);  
...
void encode_kpm_report_style5_parameterized(E2SM_KPM_IndicationMessage_t* indicationmessage,
  ...
  E2SIM_DBG_FPRINTF(stderr, "inner er encded is %ld\n", era.encoded);
  E2SIM_DBG_FPRINTF(stderr, "after encoding message\n");      
...
void encode_kpm(E2SM_KPM_IndicationMessage_t* indicationmessage) {
  ...
  asn_enc_rval_t era =
    asn_encode_to_buffer(opt_cod,
			 ...
			 &asn_DEF_SliceToReportListItem,
			 slicelistitem, e2smbuffera, e2smbuffer_sizea);
  E2SIM_DBG_FPRINTF(stderr, "inner er encded is %d\n", era.encoded);
  E2SIM_DBG_FPRINTF(stderr, "after encoding message\n");      
  ...
  asn_check_constraints(&asn_DEF_E2SM_KPM_IndicationMessage, indicationmessage, error_buf, &errlen);
  E2SIM_DBG_PRINTF("error length %d\n", errlen);
  E2SIM_DBG_PRINTF("error buf %s\n", error_buf);
  ...
  E2SIM_DBG_FPRINTF(stderr, "er encded is %d\n", er.encoded);
  E2SIM_DBG_FPRINTF(stderr, "after encoding message\n");
