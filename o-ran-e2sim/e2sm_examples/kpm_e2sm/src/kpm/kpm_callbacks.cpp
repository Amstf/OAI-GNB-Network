...
int main(int argc, char* argv[]) {
	...
	E2SIM_DBG_FPRINTF(stderr, "Starting KPM processor sim");

	E2SIM_DBG_FPRINTF(stderr, "JSON Test\n");
	...
	E2SIM_DBG_FPRINTF(stderr, "er encded is %ld\n", er.encoded);
	E2SIM_DBG_FPRINTF(stderr, "after encoding message\n");
	E2SIM_DBG_FPRINTF(stderr, "here is encoded message %s\n", e2smbuffer);

	uint8_t *ranfuncdesc = (uint8_t*)calloc(1,er.encoded);
	memcpy(ranfuncdesc, e2smbuffer, er.encoded);

	E2SIM_DBG_PRINTF("this is the char array %s\n", (char*)ranfuncdesc);
	...
	E2SIM_DBG_PRINTF("!!!lenth of ranfuncdesc is %lu\n", strlen((char*)ranfuncdesc));
	E2SIM_DBG_PRINTF("value of this index is %d\n", ranfuncdesc[0]);
	E2SIM_DBG_PRINTF("value of this index is %d\n", ranfuncdesc[1]);
	E2SIM_DBG_PRINTF("value of this index is %d\n", ranfuncdesc[2]);
	E2SIM_DBG_PRINTF("value of this index is %d\n", ranfuncdesc[3]);
	E2SIM_DBG_PRINTF("value of this index is %d\n", ranfuncdesc[4]);
...
void run_report_loop(long requestorId, long instanceId, long ranFunctionId, long actionId)
{
	...
	while ( getline(input, str) ) {
		...
		E2SIM_DBG_FPRINTF(stderr,"De line is %s\n", str.c_str());
		...
		try {
			...
			E2SIM_DBG_FPRINTF(stderr, "Exception on reading json\n");
		}

		if (valid) {

			E2SIM_DBG_FPRINTF(stderr, "key is valid\n");

			std::string first_key = all_ues_json.begin().key();
			E2SIM_DBG_FPRINTF(stderr, "first key is %s\n", first_key.c_str());

			if (first_key.compare("ueMeasReport") == 0) {

				E2SIM_DBG_FPRINTF(stderr, "it is equal to ue meas report\n");
				...
				for (int i = 0; i < numMeasReports; i++) {
					...
					E2SIM_DBG_FPRINTF(stderr,"UE number %d\n", i);
					...
					E2SIM_DBG_FPRINTF(stderr, "Throughput %d\n", prb_usage);
					...
					E2SIM_DBG_FPRINTF(stderr, "Serving Cell %d\n",nextCellId);
					...
					E2SIM_DBG_FPRINTF(stderr,"  RSRP %d\n", nextRsrp);
					...
					E2SIM_DBG_FPRINTF(stderr,"  RSRQ %d\n",nextRsrq);
					...
					E2SIM_DBG_FPRINTF(stderr,"  RSSINR %d\n", nextRssinr);
					...
					if (ueId.find("Pedestrian") != string::npos) {
						...
						E2SIM_DBG_FPRINTF(stderr, "ind is %s\n", ind.c_str());
	  ...
	  //	  uint8_t *buf2 = (uint8_t*)"12";
	    ...
	    //cout << "  RSSINR " << nextNbRssinr << endl;
					...
					E2SIM_DBG_FPRINTF(stderr,"This is neighbor str %s\n", neighbor_str.c_str());

	  //Creating UE-level RAN-Container CUCP message

					E2SIM_DBG_FPRINTF(stderr,"Creating UE-level RAN-Container CUCP message\n");
					...
					E2SIM_DBG_FPRINTF(stderr, "Now printing xer outside of function call\n");
					E2SIM_DBG_XER(stderr, &asn_DEF_E2SM_KPM_IndicationHeader, ind_header_cucp_ue);	  
					...
					E2SIM_DBG_FPRINTF(stderr, "error buf is %s\n", (char*)e2sm_header_buf_cucp_ue);
					E2SIM_DBG_FPRINTF(stderr, "after encoding header\n");	  
	  ...
	  //Creating UE-level RAN-Container CUUP message
					...
					E2SIM_DBG_FPRINTF(stderr,"Creating UE-level RAN-Container CUUP message\n");	
	  ...
	  //Creating UE-level RAN-Container DU message
					...
					E2SIM_DBG_FPRINTF(stderr,"Creating UE-level RAN-Container DU message\n");
					...
					E2SIM_DBG_FPRINTF(stderr, "done with ue meas report\n");
				}

			} else if (first_key.compare("cellMeasReport") == 0) {

				E2SIM_DBG_FPRINTF(stderr, "it is equal to cell meas report\n");
				...
				for (int i = 0; i < numMeasReports; i++) {
					...
					E2SIM_DBG_FPRINTF(stderr, "Cell ID %d\n", cellid);
					...
					E2SIM_DBG_FPRINTF(stderr, "Bytes DL %f\n", bytes_dl);
					...
					E2SIM_DBG_FPRINTF(stderr, "Bytes UL %f\n", bytes_ul);	  
					...
					E2SIM_DBG_FPRINTF(stderr, "Avail PRB DL %d\n", prb_dl);	
					...
					E2SIM_DBG_FPRINTF(stderr, "Avail PRB UL %d\n", prb_ul);
	  ...
	  //Encoding Style 5 Message Body
					...
					E2SIM_DBG_FPRINTF(stderr, "Encoding Style 5 Message body\n");
	  ...
	  //Encoding Style 1 Message Body
					...
					E2SIM_DBG_FPRINTF(stderr, "Encoding Style 1 Message body\n");	  
					asn_codec_ctx_t *opt_cod2;
					...
					*l_ul_prbs = (long)prb_ul;
					...
					E2SIM_DBG_FPRINTF(stderr, "About to encode Indication\n");
...
void encode_and_send_ric_indication_report_metrics_single(long seqNum, long requestorId, long instanceId, long ranFunctionId, long actionId) {
	...
	E2SIM_DBG_FPRINTF(stderr, "Inside encode_and_send_ric_indication_report\n");
	E2SIM_DBG_FPRINTF(stderr, "Setting cell ID to %d\n", nextCellId);
  ...
  // Encoding Style 1 Message Body
	...
	E2SIM_DBG_FPRINTF(stderr, "RIC Indication sent\n");
}
...
void callback_kpm_subscription_request(E2AP_PDU_t *sub_req_pdu) {
	...
	E2SIM_DBG_FPRINTF(stderr, "Calling callback_kpm_subscription_request\n");
  ...
  //Encode subscription response
	...
	E2SIM_DBG_XER(stderr, &asn_DEF_RICsubscriptionRequest, &orig_req);   
	...
	E2SIM_DBG_FPRINTF(stderr, "count%d\n", count);
	E2SIM_DBG_FPRINTF(stderr, "size%d\n", size);
	...
	for (int i=0; i < count; i++) {
		...
		E2SIM_DBG_FPRINTF(stderr, "The next present value %d\n", pres);

		switch(pres) {
			case RICsubscriptionRequest_IEs__value_PR_RICrequestID:
			{
				E2SIM_DBG_FPRINTF(stderr,"in case request id\n");	
				RICrequestID_t reqId = next_ie->value.choice.RICrequestID;
				requestorId = reqId.ricRequestorID;
				long instanceId = reqId.ricInstanceID;
				E2SIM_DBG_FPRINTF(stderr, "requestorId %ld\n", requestorId);
				E2SIM_DBG_FPRINTF(stderr, "instanceId %ld\n", instanceId);
			...
			{
				E2SIM_DBG_FPRINTF(stderr,"in case ran func id\n");	
				break;
			}
			case RICsubscriptionRequest_IEs__value_PR_RICsubscriptionDetails:
			{
				E2SIM_DBG_FPRINTF(stderr,"in case subscription details\n");
				RICsubscriptionDetails_t subDetails = next_ie->value.choice.RICsubscriptionDetails;
				E2SIM_DBG_FPRINTF(stderr,"in case subscription details 1\n");	
				triggerDef = &subDetails.ricEventTriggerDefinition;
				E2SIM_DBG_FPRINTF(stderr,"in case subscription details 2\n");	
				RICactions_ToBeSetup_List_t actionList = subDetails.ricAction_ToBeSetup_List;
				E2SIM_DBG_FPRINTF(stderr,"in case subscription details 3\n");

				// print trigger information
				if (triggerDef->buf) {
					E2SIM_DBG_FPRINTF(stderr, "Received trigger %s\n", triggerDef->buf);
				}
				else {
					E2SIM_DBG_FPRINTF(stderr, "No trigger received in subscription request\n");
				}

				// We identify the first action whose type is REPORT
				// That is the only one accepted; all others are rejected
				int actionCount = actionList.list.count;
				E2SIM_DBG_FPRINTF(stderr, "action count%d\n", actionCount);
				...
				for (int j=0; j < actionCount; j++) {
					...
					E2SIM_DBG_XER(stderr, &asn_DEF_E2SM_HelloWorld_ActionDefinition, test);
					...
					if (test->present == E2SM_HelloWorld_ActionDefinition_PR_actionDefinition_Format1) {
						...
						for (int z = 0; z < countRanParameters; z++) {
							...
							// OCTET_STRING_t parNameOct = list[z]->ranParameter_Name;
							// E2SIM_DBG_XER(stderr, &asn_DEF_OCTET_STRING, &parNameOct);
							// std::string parName = DecodeOctectString(&parNameOct);
							// std::cerr << "parName: " << parName << std::endl;
							// long parTest = list[z]->ranParameter_Test;
							// OCTET_STRING_t parValueOct = list[z]->ranParameter_Value;
							// std::string parValue = DecodeOctectString(&parValueOct);
							// E2SIM_DBG_XER(stderr, &asn_DEF_OCTET_STRING, &parValueOct);
					...
					E2SIM_DBG_FPRINTF(stderr, "adding accept\n");
					foundAction = true;
					} else {
						reqActionId = actionId;
						E2SIM_DBG_FPRINTF(stderr, "action rejected but not added to vector\n");
	    			// E2SIM_DBG_PRINTF("adding reject\n");
			...
			default:
			{
				E2SIM_DBG_FPRINTF(stderr,"in case default\n");	
				break;
			}      
		}
	...
	E2SIM_DBG_FPRINTF(stderr, "After Processing Subscription Request\n");
	...
	for (int i=0; i < actionIdsAccept.size(); i++) {
		E2SIM_DBG_FPRINTF(stderr, "Action ID %d %ld\n", i, actionIdsAccept.at(i));
	}
  ...
  // start report loop in a dedicated thread
	if (triggerDef->buf || (OAI_PROTOBUF && protobuf_payload_len > 0)) {
		...
		try {
			...
			E2SIM_DBG_FPRINTF(stderr, "setting trigger_timer to %d seconds for requestorId %ld\n", trigger_timer, requestorId);
			...
			E2SIM_DBG_FPRINTF(stderr, "handling exception, no valid trigger. Received %s\n", triggerDef->buf);

			if (DEBUG) {
				E2SIM_DBG_FPRINTF(stderr, "In DEBUG mode, setting trigger manually\n");
	...
	for (int idx = 0; idx < ricControlRequest->protocolIEs.list.count; idx++) {
		...
		switch (ie->value.present) {
			case RICcontrolRequest_IEs__value_PR_RICrequestID: {
				E2SIM_DBG_FPRINTF(stderr, "[E2SM] RICcontrolRequest_IEs__value_PR_RICrequestID\n");

				reqRequestorId = ie->value.choice.RICrequestID.ricRequestorID;
				reqInstanceId = ie->value.choice.RICrequestID.ricInstanceID;

				E2SIM_DBG_FPRINTF(stderr, "reqRequestorId %ld\n", reqRequestorId);
				E2SIM_DBG_FPRINTF(stderr, "reqInstanceId %ld\n", reqInstanceId);

				switch (reqRequestorId) {
					case 1001: {
						E2SIM_DBG_FPRINTF(stderr, "TS xApp message\n");
						break;
					}
					case 1002: {
						E2SIM_DBG_FPRINTF(stderr, "QoS xApp message\n");
						break;
					}
				}
				break;
			...
			case RICcontrolRequest_IEs__value_PR_RANfunctionID: {
				E2SIM_DBG_FPRINTF(stderr, "[E2SM] RICcontrolRequest_IEs__value_PR_RANfunctionID\n");
				ranFunctionId = ie->value.choice.RANfunctionID;
				E2SIM_DBG_FPRINTF(stderr, "ranFunctionId %ld\n", ranFunctionId);
				break;
			}

			case RICcontrolRequest_IEs__value_PR_RICcallProcessID: {
				E2SIM_DBG_FPRINTF(stderr, "[E2SM] RICcontrolRequest_IEs__value_PR_RICcallProcessID\n");
				reqRICcallProcessID = ie->value.choice.RICcallProcessID.buf;
				E2SIM_DBG_FPRINTF(stderr, "reqRICcallProcessID %s\n", reqRICcallProcessID);
				break;
			}
			case RICcontrolRequest_IEs__value_PR_RICcontrolHeader: {
				E2SIM_DBG_FPRINTF(stderr, "[E2SM] RICcontrolRequest_IEs__value_PR_RICcontrolHeader\n");
        // E2SIM_DBG_XER(stderr, &asn_DEF_RICcontrolHeader, &ie->value.choice.RICcontrolHeader);
				...
				E2SIM_DBG_XER(stderr, &asn_DEF_E2SM_HelloWorld_ControlHeader, e2smControlHeader);
				...
				if (e2smControlHeader->present == E2SM_HelloWorld_ControlHeader_PR_controlHeader_Format1) {
					...
					E2SIM_DBG_FPRINTF(stderr, "[E2SM] Error in checking format of E2SM Control Header\n");
				}
				break;
			}

			case RICcontrolRequest_IEs__value_PR_RICcontrolMessage: {
				E2SIM_DBG_FPRINTF(stderr, "[E2SM] RICcontrolRequest_IEs__value_PR_RICcontrolMessage\n");
        // E2SIM_DBG_XER(stderr, &asn_DEF_RICcontrolMessage, &ie->value.choice.RICcontrolMessage);
				...
				E2SIM_DBG_XER(stderr, &asn_DEF_E2SM_HelloWorld_ControlMessage, e2SmControlMessage);

				// get payload
				if (e2SmControlMessage->present == E2SM_HelloWorld_ControlMessage_PR_controlMessage_Format1)
				{
					E2SIM_DBG_FPRINTF(stderr, "[E2SM] E2SM_HelloWorld_ControlMessage_PR_controlMessage_Format1\n");
					...
					if (recvBufLen > 0) {
					  ...
					  E2SIM_DBG_FPRINTF(stderr, "Print content of RIC Control Request %s\n", ricControlRequestPayload);
        		...
        		// write policies on config file
						if (strcmp((char*) ricControlRequestPayload, "terminate") == 0) {
							...
							if (OAI_PROTOBUF) {
								for (int i = 0; i < recvBufLen; i++) {
									E2SIM_DBG_FPRINTF(stderr, " %hhx ", ricControlRequestPayload[i]);
								}
								E2SIM_DBG_FPRINTF(stderr, "\n");
						...
						E2SIM_DBG_FPRINTF(stderr, "Freeing received ricControlRequestPayload\n");
					  free(ricControlRequestPayload);
					}
					else {
						E2SIM_DBG_FPRINTF(stderr, "No payload in RIC Control Request\n");
					}
				}
				else
				{
					E2SIM_DBG_FPRINTF(stderr, "[E2SM] Error in checking format of E2SM Control Message\n");
				}
				break;
			}
			case RICcontrolRequest_IEs__value_PR_RICcontrolAckRequest: {
				E2SIM_DBG_FPRINTF(stderr, "[E2SM] RICcontrolRequest_IEs__value_PR_RICcontrolAckRequest\n");

				switch (ie->value.choice.RICcontrolAckRequest) {
					case RICcontrolAckRequest_noAck: {
						E2SIM_DBG_FPRINTF(stderr, "[E2SM] RIC Control ack value: NO ACK\n");
						break;
					}
					case RICcontrolAckRequest_ack: {
						E2SIM_DBG_FPRINTF(stderr, "[E2SM] RIC Control ack value: ACK\n");
						break;
					}
					case RICcontrolAckRequest_nAck: {
						E2SIM_DBG_FPRINTF(stderr, "[E2SM] RIC Control ack value: NACK\n");
						break;
					}
					default: {
						E2SIM_DBG_FPRINTF(stderr, "[E2SM] RIC Control ack value unknown\n");
						break;
					}
				}
				break;
			...
			case RICcontrolRequest_IEs__value_PR_NOTHING: {
				E2SIM_DBG_FPRINTF(stderr, "[E2SM] RICcontrolRequest_IEs__value_PR_NOTHING\n");
				E2SIM_DBG_FPRINTF(stderr, "[E2SM] Nothing\n");
				break;
			}
			default: {
				E2SIM_DBG_FPRINTF(stderr, "[E2SM] RIC Control value unknown\n");
				break;
			}
		}
	}
