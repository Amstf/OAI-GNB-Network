...
long encoding::get_function_id_from_subscription(E2AP_PDU_t *e2ap_pdu) {
  ...
  for (int i=0; i < count; i++) {
    ...
    E2SIM_DBG_FPRINTF(stderr, "next present value %d\n", pres);
    E2SIM_DBG_FPRINTF(stderr, "value of pres ranfuncid is %d\n", RICsubscriptionRequest_IEs__value_PR_RANfunctionID);

    if (pres == RICsubscriptionRequest_IEs__value_PR_RANfunctionID) {
      E2SIM_DBG_FPRINTF(stderr, "equal pres to ranfuncid\n");
      func_id = next_ie->value.choice.RANfunctionID;
    }
    
  }

  E2SIM_DBG_FPRINTF(stderr, "After loop, func_id is %ld\n", func_id);
...
void encoding::generate_e2apv1_setup_request_parameterized(E2AP_PDU_t *e2ap_pdu, std::vector<ran_func_info> all_funcs) {
  ...
  if (gnb_id == NULL) {
    E2SIM_DBG_FPRINTF(stderr, "Env variable GNB_ID not set. Using default values to build gNB ID\n");
    ...
    E2SIM_DBG_FPRINTF(stderr, "Using passed value to build gNB ID: %s (hex: %x)\n", gnb_id, (uint8_t*) gnb_id);
...
void encoding::generate_e2apv1_subscription_request(E2AP_PDU *e2ap_pdu) {
  ...
  E2SIM_DBG_PRINTF("error length %lu\n", errlen);
  E2SIM_DBG_PRINTF("error buf %s\n", error_buf);
...
void encoding::generate_e2apv1_subscription_response_success(E2AP_PDU *e2ap_pdu, long reqActionIdsAccepted[],
  ...
  for (int i=0; i < numAccept ; i++) {
    E2SIM_DBG_FPRINTF(stderr, "in for loop i = %d\n", i);
  ...
  E2SIM_DBG_PRINTF("ret %d\n", ret);
  E2SIM_DBG_PRINTF("error length %lu\n", errlen);
  E2SIM_DBG_PRINTF("error buf %s\n", error_buf);

  E2SIM_DBG_PRINTF("now printing xer of subscription response\n");
  E2SIM_DBG_XER(stderr, &asn_DEF_E2AP_PDU, e2ap_pdu);
  E2SIM_DBG_PRINTF("done printing xer of subscription response\n");  
...
void encoding::generate_e2apv1_subscription_response(E2AP_PDU *e2ap_pdu, E2AP_PDU *sub_req_pdu) {
  ...
  for (int i=0; i < count; i++) {
	...
	for (int i=0; i < actionCount; i++) {
	  ...
	  E2SIM_DBG_FPRINTF(stderr, "Next Action ID %ld\n", actionId);
...
void encoding::generate_e2apv1_indication_request_parameterized(E2AP_PDU *e2ap_pdu,
  ...
  E2SIM_DBG_FPRINTF(stderr, "after encoding message 1\n");

  ricind_ies7->criticality = 0;
  ricind_ies7->value.present = pres3;

  E2SIM_DBG_FPRINTF(stderr, "after encoding message 2\n");

  E2SIM_DBG_FPRINTF(stderr, "after encoding message 3\n");      
  ricind_ies7->value.choice.RICindicationMessage.size = message_length;

  E2SIM_DBG_FPRINTF(stderr, "after encoding message 4\n");
  memcpy(ricind_ies7->value.choice.RICindicationMessage.buf, ind_message_buf, message_length);

  E2SIM_DBG_FPRINTF(stderr, "after encoding message 5\n");

  uint8_t *cpid_buf = (uint8_t *)"cpid";
  OCTET_STRING_t cpid_str;

  E2SIM_DBG_PRINTF("5.1\n");
