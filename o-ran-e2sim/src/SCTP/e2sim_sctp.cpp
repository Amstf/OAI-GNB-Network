...
int sctp_accept_connection(const char *server_ip_str, const int server_fd)
{
  ...
  E2SIM_DBG_FPRINTF(stderr, "client fd is %d\n", client_fd);
  if(client_fd == -1){
    perror("accept()");
    close(client_fd);
    exit(1);
...
{
  E2SIM_DBG_FPRINTF(stderr,"in sctp send data func\n");
  E2SIM_DBG_FPRINTF(stderr,"data.len is %d", data.len);
  int sent_len = send(socket_fd, (void*)(&(data.buffer[0])), data.len, 0);
  E2SIM_DBG_FPRINTF(stderr,"after getting sent_len\n");
...
int sctp_receive_data(int &socket_fd, sctp_buffer_t &data)
{
  ...
  E2SIM_DBG_FPRINTF(stderr, "receive data1\n");
  memset(data.buffer, 0, sizeof(data.buffer));
  E2SIM_DBG_FPRINTF(stderr, "receive data2\n");  
  data.len = 0;

  //receive data from the socket
  int recv_len = recv(socket_fd, &(data.buffer), sizeof(data.buffer), 0);
  E2SIM_DBG_FPRINTF(stderr, "receive data3\n");
